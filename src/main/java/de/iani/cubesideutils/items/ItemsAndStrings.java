    ...
    public static String toNiceString(ItemStack item, int amount, String colorPrefix) {
        ...
        if (meta instanceof PotionMeta) {
            ...
            for (PotionEffect effect : potionMeta.getCustomEffects()) {
                ...
                builder.append(StringUtil.capitalizeFirstLetter(effect.getType().getName(), true))
                        .append(" ").append(StringUtil.toRomanNumber(effect.getAmplifier()));
                if (!effect.getType().isInstant()) {
                    builder.append(" (")
                            .append(StringUtil.formatTimespan(50 * effect.getDuration(), "",
                                    "", "", "", ":", ":", false, true))
                            .append(")");
                }
                index++;
        ...
        if (!enchantments.isEmpty()) {
            ...
            enchList.sort(
                    (e1, e2) -> EnchantmentNames.getName(e1).compareTo(EnchantmentNames.getName(e2)));
            ...
            for (Enchantment ench : enchList) {
                ...
                if (ench.getMaxLevel() > 1 || enchantments.get(ench) > 1) {
                    builder.append(" ")
                            .append(StringUtil.toRomanNumber(enchantments.get(ench)));
                }
